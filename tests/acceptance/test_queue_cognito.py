...
@pytest.mark.auth
def test_auth(api_client_cognito, queue_base_endpoint):
    headers = {"Authorization": None}
    assert (
        401
        == api_client_cognito.patch(
            queue_base_endpoint, json={}, headers=headers
        ).status_code
    )
    assert (
        401 == api_client_cognito.get(queue_base_endpoint, headers=headers).status_code
    )
    assert (
        401
    ...
    assert (
        401
        == api_client_cognito.delete(queue_base_endpoint, headers=headers).status_code
    )


def test_it_adds_to_queue(api_client_cognito, queue_base_endpoint, queue_table, stack):
    # Arrange
    key = "test"
    item = {
        "MatchId": key,
    ...
    # Act
    response = api_client_cognito.patch(queue_base_endpoint, json=item)
...
def test_it_adds_composite_to_queue(
    api_client_cognito, queue_base_endpoint, queue_table, stack
):
    # Arrange
    key = [
...
def test_it_adds_batch_to_queue(
    ...
    # Act
    response = api_client_cognito.patch(
...
def test_it_rejects_invalid_add_to_queue(
    api_client_cognito, queue_base_endpoint, stack
):
    scenarios = [
        {"INVALID": "PAYLOAD"},
    ...
    for scenario in scenarios:
        response = api_client_cognito.patch(queue_base_endpoint, json=scenario)
...
def test_it_gets_queue(
    api_client_cognito, queue_base_endpoint, del_queue_factory, stack
):
    # Arrange
    del_queue_item = del_queue_factory()
    # Act
    response = api_client_cognito.get(queue_base_endpoint)
    response_body = response.json()
    # Assert
...
def test_it_rejects_invalid_deletion(
    api_client_cognito, del_queue_factory, queue_base_endpoint, queue_table, stack
):
    # Arrange
    del_queue_item = del_queue_factory()
    match_id = del_queue_item["MatchId"]
    # Act
    response = api_client_cognito.delete(
...
def test_it_disables_cancel_deletion_whilst_job_in_progress(
    ...
    del_queue_item = del_queue_factory()
    deletion_queue_item_id = del_queue_item["DeletionQueueItemId"]
    response = api_client_cognito.delete(queue_base_endpoint)
    response_body = response.json()
...
def test_it_only_allows_one_concurrent_execution(
    api_client_cognito, queue_base_endpoint, sf_client, stack, execution_exists_waiter
):
    # Arrange
    # Start a job
    response = api_client_cognito.delete(queue_base_endpoint)
