...
export default () => (
  <>
    ...
    <p className="separator-top">
      Data mappers enable you to connect data hosted in S3 to the Amazon S3
      Find and Forget solution.
    </p>
    <p>
      When creating a data mapper you need to choose a table in a
      supported <strong>data catalog provider</strong> which describes the
      location and structure of the data you want to connect to the solution.
      Currently, <strong>AWS Glue</strong> is the only supported data catalog
      provider. You must also choose a <strong>query executor</strong> which is
      the service the Amazon S3 Find and Forget solution will use to query the
      data. Currently, <strong>Amazon Athena</strong> is the only supported
      query executor. After selecting a query executor, you must to choose the
